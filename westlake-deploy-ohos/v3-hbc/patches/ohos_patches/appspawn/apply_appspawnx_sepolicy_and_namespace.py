...
def main():
    ...
    # Note: OH's build_contexts.py format check enforces STRICT format:
    #   "* u:object_r:*:s0"
    # (no file type markers like `--` allowed; inline comments not allowed).
    # Lines must be: <path_pattern> <whitespace> u:object_r:<type>:s0
    fc_anchor = "/system/bin/appspawn        u:object_r:appspawn_exec:s0\n"
    fc_new = (
        "/system/bin/appspawn        u:object_r:appspawn_exec:s0\n"
        "# adapter project: Android APK spawner binary\n"
        "/system/bin/appspawn-x      u:object_r:appspawn_exec:s0\n"
    )
    ...
    sock_new = (
        ...
        "# adapter project: AppSpawnX socket (Android APK spawner)\n"
        "/dev/unix/socket/AppSpawnX         u:object_r:appspawn_socket:s0\n"
        "# adapter project: Android native lib dir (must be system_lib_file for mmap PROT_EXEC in appspawn domain)\n"
        "/system/android/lib(/.*)?          u:object_r:system_lib_file:s0\n"
    )
    ...
    # ---- 3. NOTE: ICU dir read NOT extended — using appspawn pattern ----
    # 2026-04-30 (B.45): OH design philosophy is "minimal appspawn perms;
    # delegate or rework when restricted". This mirrors the APK install
    # precedent (appspawn can't write /data/app → installD does it).
    #
    # For ICU data: appspawn already has `system_file:file read` (single
    # file mmap is permitted), but NOT `system_file:dir read` (listFiles
    # forbidden, by design — see e.g. `system_lib_file:dir { mounton getattr }`
    # only, no read). Granting dir read here would be the Zygote pattern
    # (broad-scope), not the OH appspawn pattern.
    #
    # Real fix: AppSpawnXInit.ensureICUDataLoaded uses single-file mmap
    # via java.nio.channels.FileChannel.map (within the file:read
    # permission that's already granted), then constructs ICUBinary
    # PackageDataFile via reflection. The Java path doesn't need
    # listFiles, and the appspawn domain stays minimal.
    #
    # See memory project_b44_icu_blocker_solved.md and design doc
    # ability_manager_ipc_adapter_design.html §1.1.4.4 ICU notes.
