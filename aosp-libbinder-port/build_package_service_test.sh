#!/bin/bash
# ============================================================================
# Westlake M4c -- PackageServiceTest.dex build helper
#
# Compiles aosp-libbinder-port/test/PackageServiceTest.java plus the
...
#
# Output: out/PackageServiceTest.dex
# ============================================================================
...
BUILD=/tmp/packageservicetest-build-$$
OUT_DEX="$SCRIPT_DIR/out/PackageServiceTest.dex"
...
# The test reflectively constructs com.westlake.services.WestlakePackageManagerService
...
javac -source 1.8 -target 1.8 -classpath "$ANDROID_JAR" \
    ...
    "$SCRIPT_DIR/test/PackageServiceTest.java" 2>&1 | grep -v "^Note:" || true

if [ ! -f "$BUILD/classes/PackageServiceTest.class" ]; then
    echo "ERROR: javac failed (PackageServiceTest.class missing)"
    rm -rf "$BUILD"
    exit 1
fi
...
# Package PackageServiceTest + AsInterfaceTest (and their inner classes) into
# the dex; the stubs above stay out.
mkdir -p "$BUILD/dex-only"
cp "$BUILD/classes/PackageServiceTest"*.class "$BUILD/dex-only/"
